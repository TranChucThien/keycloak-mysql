...
# Verify custom mappers for Keycloak 18+ (Quarkus) without /auth prefix
# Usage: ./verify-custom-mappers-modern.sh [KEYCLOAK_URL] [POD_NAME]
KEYCLOAK_URL="${1:-http://192.168.10.142:30080}"
POD_NAME="${2}"

echo "🔍 Verifying Custom Mappers Installation (KC18+)..."
...
if [ -n "$POD_NAME" ]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "   Build and deploy: cd versions/kc26/mapper && mvn clean package"
  fi
  echo ""
fi
...
CLIENT_RESPONSE=$(curl -s -X GET "$KEYCLOAK_URL/admin/realms/test-realm/clients?clientId=test-client" \
  -H "Authorization: Bearer $ADMIN_TOKEN")

CLIENT_UUID=$(echo "$CLIENT_RESPONSE" | jq -r '.[0].id // empty')

if [ -z "$CLIENT_UUID" ] || [ "$CLIENT_UUID" = "null" ]; then
  echo "⚠️  test-realm/test-client not found. Run setup-data-modern.sh first"
else
  MAPPERS=$(curl -s -X GET "$KEYCLOAK_URL/admin/realms/test-realm/clients/$CLIENT_UUID/protocol-mappers/models" \
    -H "Authorization: Bearer $ADMIN_TOKEN")
...
if [ -n "$CIF_MAPPER" ] && [ -n "$BRANCH_MAPPER" ]; then
  ...
  echo "   3. Rebuild image: cd versions/kc26/mapper && docker build -f Dockerfile.builtin ."
