...
# Verify script for Keycloak 12-16 (WildFly) with /auth prefix
# Usage: ./verify-data-legacy.sh [KEYCLOAK_URL]

KEYCLOAK_URL="${1:-http://192.168.10.142:30080}"
REALM_NAME="test-realm"
CLIENT_ID="test-client"

echo "🔍 Verify data for Keycloak Legacy (KC12-16)..."
...
# Check realm
echo "📝 Checking realm..."
REALM_CHECK=$(curl -s -X GET "$KEYCLOAK_URL/auth/admin/realms/$REALM_NAME" \
...
# Check users
echo "📝 Checking users..."
USER_COUNT=$(curl -s -X GET "$KEYCLOAK_URL/auth/admin/realms/$REALM_NAME/users" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq '. | length')
echo "   Total users: $USER_COUNT $([ "$USER_COUNT" -gt 0 ] && echo "✅" || echo "❌")"

# Check sample user
echo "📝 Checking sample user (customer1)..."
USER_CHECK=$(curl -s -X GET "$KEYCLOAK_URL/auth/admin/realms/$REALM_NAME/users?username=customer1" \
  -H "Authorization: Bearer $ADMIN_TOKEN")
